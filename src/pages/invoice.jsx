...
const Invoice = () => {
  ...
  const invoices = useQuery({
    queryFn: async () => {
      try {
        const result = await axios.get(
          "https://joglo-ndeso-kasir-api-dev.vercel.app/invoice?name=" + name,
          {
            withCredentials: true,
          }
        );
    ...
    mutationFn: async (item) => {
      try {
        const result = await axios.post(
          "https://joglo-ndeso-kasir-api-dev.vercel.app/invoice",
          item,
          {
            withCredentials: true,
          }
      ...
      try {
        if (item.amount > 1) {
          ...
          const result = await axios.delete(
            "https://joglo-ndeso-kasir-api-dev.vercel.app/invoice?id=" +
              item.id,
            { withCredentials: true }
          );
          return result;
    ...
    Swal.fire({
      ...
      .then(async (item) => {
        if (item.isConfirmed) {
          try {
            const result = await axios.delete(
              "https://joglo-ndeso-kasir-api-dev.vercel.app/invoice/invoice-name?id=" +
                params.id,
              { withCredentials: true }
            );
            Swal.fire({
  ...
  const handleBayarMutation = useMutation({
            ...
            if (item.isConfirmed) {
              try {
                ...
                const bayarInvoice = await axios.post(
                  "https://joglo-ndeso-kasir-api-dev.vercel.app/orders",
                  { data: order },
                  { withCredentials: true }
                );
                await axios.delete(
    ...
    //   try {
    //     const result = await axios.delete(
    //       "https://joglo-ndeso-kasir-api-dev.vercel.app/invoice/invoice-name?id=" + params.id,
    //       { withCredentials: true }
    //     );
    //     return result;
    //   } catch (error) {
